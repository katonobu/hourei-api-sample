...
import ViewLawMenuButton from "@/components/law/view-info-button";
...
import { DownloadLawDropdown } from "@/components/law/download-law";

export const metadata: Metadata = {
  title: "法令画面 | 法令APIプロトタイプ利用サンプル",
  description: "法令画面",
};
...
export default async function Law({
  ...
  logger.info({
    message: `[law] getLawData: ${end - start}ms`,
  });
  ...
  if (result.isSuccess) {
    ...
    logger.info({
      message: `[law] getLawComponentData: ${end2 - start2}ms`,
    });
    ...
    logger.info({
      message: `[law] render: ${endRender - startRender}ms`,
    });

    return (
      <>
        {/** 法令種別,法令名,法令番号 */}
        <div className="md:relative fixed z-20 flex items-center w-full h-12 bg-White-1000 border-b md:border-none border-light-Border-Divider px-3">
