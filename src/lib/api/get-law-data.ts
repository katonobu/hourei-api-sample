...
export const getLawData = async ({
  ...
  try {
    ...
    logger.info({ message: `[API] lawdata response time: ${end - start}ms` });
    ...
    const xp = new XMLParser({
      ...
      trimValues: false, // trueの場合、タグまたは属性値から周囲の空白を削除（先頭の全角スペースも削除対象）
      tagValueProcessor: (tagName, val) => (val.trim() === "" ? "" : val), // タグ値の解析処理（空項目の削除処理）
    });
    ...
    logger.info({
      message: `[API] lawdata xml parse time: ${endParse - startParse}ms`,
    });
    ...
    logger.info({
      message: `[API] lawdata xml convert time: ${endConvert - startConvert}ms`,
    });
    ...
    const startAttachedFiles = performance.now();
    const attachedFilesInfo = parsedXmlToAttachedFilesInfo(
      lawDataResponse.lawdata_response[1] as ParsedAttachedFilesInfo
    );
    const endAttachedFiles = performance.now();
    logger.info({
      message: `[API] lawdata xml attachedFilesInfo convert time: ${
        endAttachedFiles - startAttachedFiles
      }ms`,
    });

    const startLawInfo = performance.now();
    // parseしたXMLをlawInfoの型に変換
    const lawInfo = parsedXmlToLawInfo(
      lawDataResponse.lawdata_response[2] as ParsedLawInfo
    );
    const endLawInfo = performance.now();
    logger.info({
      message: `[API] lawdata xml lawInfo convert time: ${
        endLawInfo - startLawInfo
      }ms`,
    });

    const startRevisionInfo = performance.now();
    ...
    const endRevisionInfo = performance.now();
    logger.info({
      message: `[API] lawdata xml revisionInfo convert time: ${
        endRevisionInfo - startRevisionInfo
      }ms`,
    });
