...
import { DownloadLawDropdown } from "../law/download-law";
 ...
 * @param {boolean} props.isLast - 最終表示法令データ
 * @returns {JSX.Element} - 検索結果の法令データ
 */
const SearchResultItem = ({ item, asOf, isLast }: SearchResultItemProps) => {
  ...
  const downloadLawTitle: LawTitleType | undefined = item.revisionInfo?.lawTitle
    ? { LawTitle: [{ _: item.revisionInfo?.lawTitle }], ":@": {} }
    : undefined;
  const downloadLawNum: LawNumType | undefined = item.lawInfo?.lawNum
    ? { LawNum: [{ _: item.lawInfo?.lawNum }] }
    : undefined;
    ...
    >
      ...
      <div className="flex flex-col px-4 gap-2 md:flex-row items-start justify-between">
        ...
        <div>
          ...
          <LawDateLabel
            label={"公布日"}
            date={item.lawInfo?.promulgationDate}
          />
          {/** 施行日 */}
          <LawDateLabel
            label={"施行日"}
      ...
      <div className="flex w-full px-4">
        {/** 沿革情報リンク */}
        <Link
          className="text-light-Text-Body flex items-center justify-start md:justify-end mr-auto"
        ...
        <DownloadLawDropdown
          lawRevisionId={item.revisionInfo?.lawRevisionId ?? ""}
          lawTitle={downloadLawTitle}
          lawNum={downloadLawNum}
          lawType={item.lawInfo?.lawType}
          dialogClassName={isLast ? "-top-80" : ""}
        />
      </div>
      {isShow && (
        <AmendmentInfo
